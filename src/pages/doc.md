...
## **2.1 Cliente — Visualização e Interatividade**

O Client é a interface principal do GDN. Ele exibe o mapa 3D, recebe atualizações contínuas e mantém a experiência fluida mesmo com centenas de entidades móveis. Tudo isso acontece a partir de uma WebScene ArcGIS renderizada em React, sincronizada com os dados recebidos via SSE — transmissões contínuas que evitam requisições repetidas.

---

## **2.2 Servidor — Ingestão, Processamento e Transmissão de Estado em Tempo Real**

O servidor coleta, trata e cruza todas as fontes de dados que compõem o GDN. É aqui que acontece a lógica geoespacial — desde a ingestão de APIs externas até o processamento geoespacial assíncrono e armazenamento dos dados.  
Cada atualização consolida o estado da cidade, cruzando ônibus, alertas, tráfego e polígonos de Voronoi, antes de armazenar tudo em um banco de acesso rápido.

---

## **2.3 Pipelines Automáticas — Orquestração de Fluxos ETL**

A automação do GDN é realizada pelo Prefect, responsável por manter o sistema atualizado. A cada ciclo, ele coleta informações externas, normaliza os dados, trata falhas e atualiza as camadas ArcGIS utilizadas por todo o Escritório de Dados.  
Essas pipelines funcionam como o coração do sistema: bombeiam os dados que alimentam todo o ecossistema, garantindo que o estado urbano esteja sempre fresco, válido e pronto para ser consumido pela aplicação.

---

## 2.4 Resumo da Plataforma

O GDN combina:

- **mapas 3D avançados**
- **streaming de dados**
- **pipelines ETL automatizadas**
- **processamento geoespacial assíncrono**

resultando em uma solução moderna e preparada para evoluir conforme a cidade cresce e novas integrações se tornam necessárias.
...
## **3.5. Estados de Atenção e Informações no Painel**

Os estados de "Atenção" exibidos no painel resultam da **combinação** de diferentes fatores:
